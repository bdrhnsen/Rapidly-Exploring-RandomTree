...
def cluster_generate(i,j,k,radius,p):
    ...
    #     circumfrance_points.append(point_generate(cir_point[0],cir_point[1],cir_point[2],p))
        #Obstacles_list.append(point_generate(cir_point[0],cir_point[1],cir_point[2],p))
...
def determine_cluster(Obstacles_list):
    ...
    draw_wall(Obstacles_list,clustered_points,counter_list,labels_list)
#ctrl shift 7 , ctrl u k

def draw_wall(Obstacles_list,clustered_points,counter_list,labels_list):#clustered_list [x,y,z,label] şeklinde ve labellara göre sıralı
    ...
    for p in range(len(clustered_points)):
        if clustered_points[p][1]== temp:
            ...
            radius=find_cluster_radius(a_list,center)
